# MODIFIED FROM https://github.com/Qiskit/qiskit-terra/blob/stable/0.20/qiskit/utils/quantum_instance.py
# All modifications will be encased in comments CHANGE BEGIN and CHANGE END
...
class QuantumInstance:
    ...
    def __init__(
        ...
        # CHANGE BEGIN
        self.mthree = mthree
        self.mthree_timestamp = None
        self.mthree_mit = None
        # CHANGE END
    ...
    # CHANGE BEGIN
    # Scale factor for ZNE
    def execute(self, circuits, had_transpiled: bool = False, scale_factor = None):
        """
        ...
        # Bypass gates Mitiq does not support
        from ..utils import fold_gates
        from qiskit.compiler import transpile

        if scale_factor is not None:
            circuits = [fold_gates(circ, scale_factor) for circ in circuits]
            # Keep optimization_level = 0 to maintain structure
            circuits = transpile(circuits, self._backend, optimization_level=0)
    # CHANGE END
            ...
            if meas_error_mitigation_fitter is not None:
                ...
                for qubit_index_str, c_idx in qubit_mappings.items():
                    ...
                    for i, n in enumerate(c_idx):
                        ...
                        # CHANGE BEGIN
                        # we should not be rounding here to conserve as much accuracy as possible
        ...
        # CHANGE BEGIN
        # implement matrix-free error mitigation
        elif self.mthree:
            result = (
                run_circuits(
                    circuits,
            ...
            if self.mthree_mit is None:
                self.mthree_mit = mthree.M3Mitigation(self._backend)
            mit = self.mthree_mit
            ...
            for i in range(len(counts)):
                ...
                if self.mthree_timestamp is None:
                    mit.cals_from_system(qubit_index)
                    self.mthree_timestamp = time.time()
                current_timestamp = time.time()
                if (current_timestamp - self.mthree_timestamp) / 60 > self._cals_matrix_refresh_period:
                    mit.cals_from_system(qubit_index)
                    self.mthree_timestamp = time.time()
                corrected_counts = mit.apply_correction(counts[i], qubit_index)
                shots = corrected_counts.shots
                # Normalize (no negative probabilities)
                d = dict(corrected_counts)
                m = min(list(d.values()))
                if m < 0:
                    add = abs(m)
                    tot = sum(list(d.values())) + add * len(d.values())
                    for k in d:
                        d[k] = (d[k] + add) / tot
                # Rescale
                for k in result.results[i].data.counts:
                    bit_k = bin(int(k, 16))[2:].zfill(len(qubit_index))
                    result.results[i].data.counts[k] = d[bit_k] * shots
