# MODIFIED FROM https://github.com/Qiskit/qiskit-terra/blob/stable/0.20/qiskit/opflow/converters/circuit_sampler.py
...
class CircuitSampler(ConverterBase):
    ...
    def convert(
        ...
        sampled_statefn_dicts = self.sample_circuits(circuit_sfns=circs, param_bindings=p_b, scale_factor=scale_factor)
        # CHANGE END
    ...
    def sample_circuits(
        ...
        results = self.quantum_instance.execute(
            ready_circs, had_transpiled=self._transpile_before_bind, scale_factor=scale_factor
        )
        # CHANGE END
